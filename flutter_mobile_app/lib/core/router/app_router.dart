...
import 'package:smart_parking_mobile/features/customer_packages/views/screens/booking_details_list_screen.dart';
import 'package:smart_parking_mobile/features/customer_packages/views/screens/draft_list_screen.dart';
...
import 'package:smart_parking_mobile/features/customer_account/views/screens/group_members_screen.dart';
...
import 'package:smart_parking_mobile/features/customer_payment/views/screens/resume_payment_screen.dart';
...
abstract class AppRoutes {
  ...
  static const groupMembers = '/customer/group-members';
  ...
  static const bookingDetailsList = '/customer/booking-details-list';
  static const draftList = '/customer/draft-list';
  // Parking sessions
...
GoRouter buildAppRouter(BuildContext context, AuthViewModel authViewModel) {
  return GoRouter(
    ...
    redirect: (context, state) async {
      ...
      final isAuthRoute =
          state.matchedLocation == AppRoutes.loginPhone ||
    ...
    routes: [
      ...
      GoRoute(
        ...
        builder: (context, state) => const LoginFlowScreen(),
      ),
        ...
        routes: [
          GoRoute(
            ...
            pageBuilder: (context, state) =>
                const NoTransitionPage(child: CustomerHomeScreen()),
          ),
          GoRoute(
          ...
          GoRoute(
            ...
            pageBuilder: (context, state) =>
                const NoTransitionPage(child: CustomerHistoryScreen()),
          ),
          GoRoute(
            path: AppRoutes.accountMenu,
            pageBuilder: (context, state) =>
                const NoTransitionPage(child: AccountMenuScreen()),
          ),
          GoRoute(
            path: AppRoutes.parkingSessionList,
            pageBuilder: (context, state) =>
                const NoTransitionPage(child: ParkingSessionListScreen()),
          ),
        ],
      ),
      ...
      GoRoute(
        ...
        builder: (context, state) =>
            GroupDetailScreen(customerId: state.pathParameters['id'] ?? ''),
      ),
      GoRoute(
        path: AppRoutes.groupEdit,
        builder: (context, state) =>
            GroupEditScreen(groupId: state.pathParameters['id'] ?? ''),
      ),
      GoRoute(
      ...
      GoRoute(
        ...
        builder: (context, state) {
          ...
          if (state.extra is Map<String, dynamic>) {
            ...
            hideActions = extra['hideActions'] == true;
          }
          return BookingDetailItemScreen(
            bookingDetailId: id,
      ...
      GoRoute(
        ...
        builder: (context, state) => RenewPackageScreen(
          bookingDetailId: state.pathParameters['id'] ?? '',
        ),
      ),
      GoRoute(
        ...
        builder: (context, state) {
          ...
          final customerId = extra is int
              ? extra
              : int.tryParse(extra?.toString() ?? '') ?? 0;
          return AddVehicleScreen(bookingId: bookingId, customerId: customerId);
        },
      ),
      GoRoute(
        path: AppRoutes.bookingDetailsList,
        builder: (context, state) {
          final extra = state.extra as Map<String, dynamic>?;
          return BookingDetailsListScreen(
            status: extra?['status'] as String?,
            title: extra?['title'] as String? ?? 'Danh sách xe',
          );
        },
      ),
      GoRoute(
      ...
      GoRoute(
        ...
        builder: (context, state) => ParkingSessionDetailScreen(
      ...
      GoRoute(
        ...
        builder: (context, state) {
          final extra = state.extra;
          final ids = extra is List<int> ? extra : <int>[];
          return QRPaymentScreen(bookingDetailIds: ids);
        },
      ),
      GoRoute(
        path: '/payment/resume',
        builder: (context, state) {
          final payment = state.extra as PaymentResponse;
          return ResumePaymentScreen(payment: payment);
        },
      ),
      GoRoute(
