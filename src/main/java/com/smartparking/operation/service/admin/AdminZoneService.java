...
public class AdminZoneService {
    ...
    public Zone getZoneById(Integer id) {
        return zoneRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Lỗi: Không tìm thấy Khu vực với ID: " + id));
    }
    ...
    public Zone createZone(Zone zone) {
        ...
        if (zone.getParentZoneId() != null) {
            if (!zoneRepository.existsById(zone.getParentZoneId())) {
                throw new RuntimeException("Lỗi: Khu vực cha không tồn tại!");
            }
        }
    ...
    public Zone updateZone(Integer id, Zone updates) {
        ...
        if (updates.getParentZoneId() != null && updates.getParentZoneId().equals(id)) {
            throw new RuntimeException("Lỗi logic: Một khu vực không thể tự làm cha của chính nó!");
        }
...
//        existing.setCurrentOccupancy(updates.getCurrentOccupancy());
    ...
    public void deleteZone(Integer id) {
        ...
        if (zoneRepository.existsByParentZoneId(id)) {
            throw new RuntimeException("Không thể xóa: Khu vực này đang chứa các khu vực con. Vui lòng xóa khu vực con trước!");
        }

        if (ioTDeviceRepository.existsByZoneId(id)) {
            throw new RuntimeException("Không thể xóa: Đang có thiết bị IoT lắp đặt tại khu vực này!");
        }

        if (existing.getCurrentOccupancy() != null && existing.getCurrentOccupancy() > 0) {
            throw new RuntimeException("Không thể xóa: Khu vực này hiện đang có xe đỗ!");
        }
