...
public class MasterCustomerController {
    ...
    private Integer getGroupId(CustomAccountPrincipal principal) {
        if (principal.getMemberGroupIds() == null || principal.getMemberGroupIds().isEmpty()) {
            throw new AccessDeniedException("Lỗi: Bạn không thuộc bất kỳ nhóm nào!");
        }
        Object idObj = principal.getMemberGroupIds().get(0);
    ...
    public ResponseEntity<ApiResponse<PageResponse<CustomerResponse>>> listCustomers(
        ...
        Integer groupId = getGroupId(principal);
