...
public class AdminSubscriptionService {
    ...
    public VehicleType getVehicleTypeById(Integer id) {
        return vehicleTypeRepository.findById(id)
                .orElseThrow(() -> new BusinessException("Lỗi: Không tìm thấy loại phương tiện với ID: " + id));
    }
    ...
    public VehicleType createVehicleType(VehicleType vehicleType) {
        ...
        if (vehicleTypeRepository.existsByTypeCode(vehicleType.getTypeCode())) {
            throw new BusinessException("Lỗi: Mã phương tiện '" + vehicleType.getTypeCode() + "' đã tồn tại!");
        }
    ...
    public VehicleType updateVehicleType(Integer id, VehicleType vehicleTypeUpdates) {
        ...
        if (!existing.getTypeCode().equalsIgnoreCase(vehicleTypeUpdates.getTypeCode()) &&
            ...
            throw new BusinessException("Lỗi: Mã phương tiện '" + vehicleTypeUpdates.getTypeCode() + "' đã được sử dụng!");
        }
    ...
    public void deleteVehicleType(Integer id) {
        ...
        try {
            ...
            throw new BusinessException("Lỗi: Không thể xóa loại xe này vì đang có Gói cước hoặc Bảng giá sử dụng nó!");
        }
    }

    public Page<Package> getPackages(String searchName, Boolean status,Integer groupProfileId, Pageable pageable) {
        String finalSearch = StringUtils.hasText(searchName) ?
                "%" + searchName.trim().toLowerCase() + "%" : "%";

        return packageRepository.filterDynamic(finalSearch, status,groupProfileId, pageable);
    }

    public Package getPackageById(Integer id) {
        return packageRepository.findById(id)
                .orElseThrow(() -> new BusinessException("Lỗi: Không tìm thấy gói cước với ID: " + id));
    }

    @Transactional
    public Package createPackage(Package pkg) {
        if (packageRepository.existsByPackageCode(pkg.getPackageCode())) {
            throw new BusinessException("Lỗi: Mã gói cước '" + pkg.getPackageCode() + "' đã tồn tại!");
        }
    ...
    public Package updatePackage(Integer id, Package packageUpdates) {
        ...
        if (!existing.getPackageCode().equalsIgnoreCase(packageUpdates.getPackageCode()) &&
            ...
            throw new BusinessException("Lỗi: Mã gói cước '" + packageUpdates.getPackageCode() + "' đã được sử dụng!");
        }
    ...
    public void deletePackage(Integer id) {
        ...
        try {
            ...
            throw new BusinessException("Lỗi: Không thể xóa Gói cước này vì đã có dữ liệu Bảng Giá hoặc Khách Hàng đang sử dụng!");
        }
    }
    ...
    public PackageVehicleType createPackageVehicleType(PackageVehicleType entity) {
        if (packageVehicleTypeRepository.existsByPackageIdAndVehicleTypeId(
            ...
            throw new BusinessException("Lỗi: Loại phương tiện này đã được cấu hình trong gói cước rồi!");
        }
    ...
    public PackageVehicleType updatePackageVehicleType(Integer id, PackageVehicleType updates) {
        ...
        if (!existing.getVehicleTypeId().equals(updates.getVehicleTypeId()) &&
            ...
            throw new BusinessException("Lỗi: Loại phương tiện này đã tồn tại trong gói cước, không thể đổi sang!");
        }
